...
bool button_is_pressed(ButtonId id) {
  ...
  if (button_config->gpiote.gpio_pin == GPIO_Pin_NULL) return false; // not a real button
...
void button_init(void) {
  ...
  for (int i = 0; i < NUM_BUTTONS; ++i) {
    if (BOARD_CONFIG_BUTTON.buttons[i].gpiote.gpio_pin == GPIO_Pin_NULL) continue; // ignore fake buttons
