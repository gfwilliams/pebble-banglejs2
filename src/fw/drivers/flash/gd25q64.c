...
static QSPIFlashPart QSPI_FLASH_PART = {
  .instructions = {
    ...
    .exit_low_power = 0xAB,
    .enter_quad_mode = 0x35, // was 0x38 - 35 should be a no-op (read status reg)
    .reset_enable = 0x66,
    .reset = 0x99,
...
status_t flash_impl_init(bool coredump_mode) {
  ...
  // On Bangle.js we have to poke flash memory repeatedly to get it to wake up (not sure this helps right now)
